...
with col1:
    ...
    department = st.selectbox(
        "Department", 
        options=["Computer Science", "Engineering", "Business", "Arts", "Science"]
    )
...
with col2:
    ...
    assignments = st.number_input("Assignments Completed", min_value=0, max_value=100, value=10, step=1)
...
if st.button("🔮 Predict Result", type="primary", use_container_width=True):
    # Construct input dataframe matching the exact 8 feature names expected by the model
    ...
    try:
        ...
        if str(prediction).strip().lower() == "pass":
            st.success(f"🎉 **Result:** The student is predicted to **PASS**!")
        else:
            st.error(f"⚠️ **Result:** The student is predicted to **FAIL**.")
        ...
        st.info(
            "💡 **Note:** If your model was trained with categorical encoding (e.g., One-Hot Encoding or Label Encoding) "
            "for `Gender` or `Department`, ensure the input values match the exact format used during training."
        )
